...
def analyze_run(run_text):
    ...
    # Extract Final Quality using the [SOLUTION ANALYSIS] marker
...
def analyze_all_runs(file_path):
    """
    Extracts complete run blocks using a regex that captures from the run header 
    up to the corresponding "End Time:" line.
    """
    with open(file_path, 'r', encoding='utf-8') as f:
        file_content = f.read()
    # Capture each run block from header to "End Time:" (inclusive)
    pattern = re.compile(r"(\[Run \d+\].*?End Time:.*?)(?=\n\[Run \d+\]|$)", re.DOTALL | re.MULTILINE)
    run_blocks = pattern.findall(file_content)
    all_summaries = [analyze_run(block.strip()) for block in run_blocks]
    return all_summaries

def print_run_summaries(run_summaries):
    summary_text = ""
    for summary in run_summaries:
        summary_text += f"📊 MACO Test Run {summary.get('run_number', 'N/A')} – Summary\n"
        summary_text += f"  ✔ Best Quality: {summary.get('best_q', 'N/A')}\n"
        summary_text += f"  ✔ Final Quality: {summary.get('final_q', 'N/A')}\n"
        summary_text += f"  ✔ Average Quality: {summary.get('avg_q', 'N/A')}\n"
        summary_text += f"  ✔ Entropy Evolution: {summary.get('entropy_evolution', 'N/A')}\n"
        summary_text += f"  ✔ Alpha Growth: {summary.get('alpha_growth', 'N/A')}\n"
        summary_text += f"  ✔ Rho Stability: {summary.get('rho_stability', 'N/A')}\n"
        summary_text += f"  ✔ Noise_std Effect: {summary.get('noise_std_effect', 'N/A')}\n"
        summary_text += f"  ✔ Partial Reset Triggers: {summary.get('reset_triggers', 'N/A')}\n"
        summary_text += f"  ✔ MiniSat Verdict: {summary.get('minisat_verdict', 'N/A')}\n"
        summary_text += f"  ✔ MiniSat Conflicts: {summary.get('minisat_conflicts', 'N/A')}\n"
        summary_text += f"  ✔ MiniSat Propagations: {summary.get('minisat_propagations', 'N/A')}\n"
        if summary.get("unsat_clauses") is not None and summary.get("best_q", 0) > 0.99:
            summary_text += f"  ⚠ Potential ε-SAT Case: {summary['unsat_clauses']} clauses unsatisfied.\n"
        summary_text += "\n"
    # Print to console
    print(summary_text)
    # Write to file
    with open("run_summaries.txt", "w", encoding="utf-8") as f:
